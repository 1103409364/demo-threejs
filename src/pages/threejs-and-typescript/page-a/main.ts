...
const envTexture = new CubeTextureLoader().load(
  [getImg("px_50"), getImg("nx_50"), getImg("py_50"), getImg("ny_50"), getImg("pz_50"), getImg("nz_50")],
  () => {
    material.envMap = pmremGenerator.fromCubemap(envTexture).texture;
    pmremGenerator.dispose(); // 释放资源
    scene.background = material.envMap; // 设置背景
  },
);
...
export const pageInfo = { title: "MeshPhysicalMaterial" };
