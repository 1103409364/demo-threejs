...
class CSG {
  ...
  static fromMesh = function (mesh: THREE.Mesh, objectIndex?: object) {
    ...
    for (let i = 0; i < csg.polygons.length; i++) {
      ...
      for (let j = 0; j < p.vertices.length; j++) {
        ...
        v.pos.copy(
          CSG.ttvv0.copy(new THREE.Vector3(v.pos.x, v.pos.y, v.pos.z)).applyMatrix4(mesh.matrix),
        );
        v.normal.copy(
          CSG.ttvv0.copy(new THREE.Vector3(v.normal.x, v.normal.y, v.normal.z)).applyMatrix3(CSG.tmpm3),
        );
      }
    }
    return csg;
  ...
  static toMesh = function (csg: CSG, toMatrix: THREE.Matrix4, toMaterial?: THREE.Material) {
    ...
    const geom: THREE.BufferGeometry = new THREE.BufferGeometry();
    ...
    switch (polygonType) {
      ...
      case SPANNING:
        ...
        for (let i = 0; i < polygon.vertices.length; i++) {
          ...
          if ((ti | tj) == SPANNING) {
            const t = (this.w - this.normal.dot(vi.pos)) / this.normal.dot(tv0.copy(vj.pos).sub(vi.pos));
